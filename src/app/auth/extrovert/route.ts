...
export async function GET(request: Request) {
  ...
  const returnTo = bridge.return_to.startsWith("/app/messages/social/") ? bridge.return_to : "/app/messages";
  ...
  await admin.from("extrovert_datebu_auth_bridges").update({ consumed_at: new Date().toISOString() }).eq("id", bridge.id).is("consumed_at", null);

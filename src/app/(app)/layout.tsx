...
export default async function AppLayout({children}:{children:React.ReactNode}){
 ...
 if(!extrovertProfile?.profile_completed){const extrovert=(process.env.EXTROVERT_URL||"http://localhost:3000").replace(/\/$/,"");redirect(`${extrovert}/app/setup`);}
 if(extrovertProfile.trust_state==="banned")redirect(routes.login);
 const userRole=(profile?.role??"").toUpperCase(); const ownerId=process.env.DATEBU_OWNER_ID?.trim(); const adminEmails=(process.env.SUPER_ADMIN_EMAILS??"").split(",").map(e=>e.trim().toLowerCase()).filter(Boolean);
 ...
 return <div className="flex h-[100dvh] flex-col overflow-visible bg-white overscroll-none select-none"><PushNotifications/><PersonalDashboardShortcut/><MessageKeyBootstrap userId={userId}/><AppNavbar userEmail={userEmail} isSuperAdmin={isSuperAdmin}/><main className="min-h-0 flex-1 overflow-y-auto">{children}</main></div>;
}

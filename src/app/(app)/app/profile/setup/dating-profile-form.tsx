...
export function DatingProfileForm({ userId, interests, existingProfile, existingPhotoUrls, existingPhotoPaths, existingInterestIds, existingPreferences, identity }: Props) {
  ...
  const [minAge, setMinAge] = useState(Math.max(18, existingPreferences?.min_age ?? 18));
  const [maxAge, setMaxAge] = useState(Math.max(18, existingPreferences?.max_age ?? 30));
  ...
  function submit(event: FormEvent<HTMLFormElement>) { if (minAge < 18 || maxAge < 18 || minAge > maxAge) { event.preventDefault(); setNotice("Choose a valid dating age range from 18 to 60."); } }
  ...
  return <form action={formAction} onSubmit={submit} className="space-y-4">
    ...
    <Card className="p-4 sm:p-5"><Title icon={<GraduationCap className="h-4 w-4 text-emerald-600" />} title="What do you do?" subtitle="Education and work context helps people understand you quickly." /><div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2"><ReadOnly label="College / university" value="Your verified campus" /><ReadOnly label="Study / work" value={`${identity.department || "Student"} · ${identity.academicYear || "Current"}`} /></div><p className="mt-2 text-[10px] leading-4 text-muted-foreground">College / university and student identity changes belong in Extrovert.</p></Card>

...
type ViewCountDb = { from: (table: "profile_views") => { select: (columns: string, options?: { count?: "exact"; head?: boolean }) => { eq: (column: string, value: string): Promise<{ count: number | null }> } } };
...
export default async function ProfileViewsPage() {
  ...
  const [{ data: subscription }, { count }] = await Promise.all([
    ...
    (supabase as unknown as ViewCountDb).from("profile_views").select("id", { count: "exact", head: true }).eq("viewed_id", user.id),
  ]);
  ...
  const { data: rawViewers, error } = await supabase.rpc("get_profile_viewers", { p_limit: 100 });
  if (error) console.error("Failed to load profile viewers:", error);
  const viewers = (rawViewers ?? []) as Viewer[];

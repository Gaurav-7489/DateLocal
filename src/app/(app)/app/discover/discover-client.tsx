...
export default function DiscoverClient({profiles,isPro=false}:Props){
 ...
 // Keep only the next two images warm so the deck stays light on memory and decode work.
 useEffect(()=>{
 ...
 const superChatPrice=SHOP_PRODUCTS.superchat.amountPaise/100;
 ...
 return <div className="relative mx-auto flex h-[calc(100dvh-3.5rem)] md:h-[calc(100vh-4rem)] w-full max-w-md flex-col justify-between px-3 pt-1 pb-20 md:pb-4 font-sans overflow-hidden select-none">
  ...
  {current&&<div className="shrink-0 mx-auto flex w-full max-w-[348px] items-end justify-center gap-2 rounded-[2rem] border border-zinc-200 bg-white/95 px-2.5 py-2 shadow-[0_12px_35px_rgba(15,23,42,.10)] backdrop-blur-xl z-30">
   <button type="button" onClick={()=>void rewind()} disabled={!isPro||loading} className="group flex h-14 w-14 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full border border-zinc-200 bg-white text-zinc-500 shadow-sm transition-all active:scale-90 disabled:opacity-35" aria-label={isPro?"Undo last pass":"Undo is Extrovert"} title={isPro?"Undo last pass":"Undo is an Extrovert feature"}><RotateCcw className="h-4.5 w-4.5 transition-transform group-hover:-rotate-12"/><span className="text-[8px] font-bold leading-none">Undo</span></button>
   <button type="button" disabled={loading} onClick={()=>void like(current.id)} className="group flex h-14 w-14 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full bg-emerald-600 text-white shadow-md shadow-emerald-600/25 transition-all active:scale-90 disabled:opacity-50" aria-label="Like profile" title="Like"><Heart className="h-4.5 w-4.5 fill-current transition-transform group-hover:scale-110"/><span className="text-[8px] font-black leading-none">Like</span></button>
   <button type="button" disabled={loading} onClick={()=>void superLike(current.id)} className="group flex h-14 w-14 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full border border-orange-200 bg-white text-orange-500 shadow-sm transition-all active:scale-90 disabled:opacity-50" aria-label="Super Like" title="Super Like"><Star className="h-4.5 w-4.5 fill-current transition-transform group-hover:scale-110"/><span className="text-[8px] font-black leading-none">Super Like</span></button>
   <button type="button" disabled={loading} onClick={()=>setSuperChatProfile(current)} className="group flex h-14 w-14 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full bg-violet-600 text-white shadow-md shadow-violet-600/25 transition-all active:scale-90 disabled:opacity-50" aria-label={`SuperChat · ₹${superChatPrice}`} title={`SuperChat · ₹${superChatPrice}`}><MessageCircle className="h-4.5 w-4.5 transition-transform group-hover:scale-110"/><span className="text-[8px] font-black leading-none">SuperChat</span></button>
   <Link href={`${routes.profileView}/${current.id}`} className="group flex h-14 w-14 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full border border-zinc-300 bg-white text-zinc-700 shadow-sm transition-all active:scale-90" aria-label="View profile" title="View profile"><UserRound className="h-4.5 w-4.5 transition-transform group-hover:scale-110"/><span className="text-[8px] font-black leading-none">Profile</span></Link>
  </div>}
  <AnimatePresence>{safety&&current&&<div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/55 p-3 backdrop-blur-sm sm:items-center sm:p-4"><motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:20}} className="max-h-[calc(100dvh-1.5rem)] w-full max-w-sm overflow-y-auto rounded-[2rem] border border-white/15 bg-card p-5 shadow-2xl"><div className="flex items-center justify-between border-b border-border pb-3"><span className="text-sm font-black text-foreground">Safety Controls</span><button type="button" onClick={()=>setSafety(false)} className="rounded-full border border-border bg-background p-2 text-muted-foreground active:scale-95"><X className="h-4 w-4"/></button></div><div className="mt-3 space-y-2.5"><Link href={`${routes.profileView}/${current.id}`} onClick={()=>setSafety(false)} className="flex w-full items-center gap-3 rounded-2xl border border-border bg-muted/50 p-3.5 text-xs font-bold text-foreground"><UserRound className="h-4 w-4 text-muted-foreground"/>View full profile</Link><button type="button" onClick={()=>{setSafety(false);setReportProfile(current)}} className="flex w-full items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs font-bold text-amber-700"><Flag className="h-4 w-4"/>Report profile</button><button type="button" onClick={()=>void block(current)} className="flex w-full items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-3.5 text-xs font-bold text-rose-700"><UserX className="h-4 w-4"/>Block user</button></div></motion.div></div>}</AnimatePresence>
  <AnimatePresence>{reportProfile&&<div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 p-3 backdrop-blur-sm sm:p-4"><motion.div initial={{scale:.96,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:.96,opacity:0}} className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-y-auto rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><h3 className="text-base font-black text-zinc-950"><Flag className="mr-1.5 inline h-4 w-4 text-rose-600"/>Report {reportProfile.display_name}</h3><button type="button" onClick={()=>setReportProfile(null)} className="rounded-full border border-zinc-200 p-2 text-zinc-500 hover:bg-zinc-100"><X className="h-4 w-4"/></button></div><form onSubmit={report} className="mt-4 space-y-3"><select value={reportReason} onChange={e=>setReportReason(e.target.value)} className="w-full rounded-xl border border-zinc-200 bg-white p-3 text-xs text-zinc-900 outline-none focus:border-emerald-500"><option>Inappropriate photo or content</option><option>Harassment or abusive behavior</option><option>Fake or impersonated profile</option><option>Spam or commercial advertising</option><option>Underage user</option><option>Other safety concern</option></select><textarea value={reportDetails} onChange={e=>setReportDetails(e.target.value)} rows={4} maxLength={500} placeholder="Additional details (optional)" className="w-full resize-none rounded-xl border border-zinc-200 bg-white p-3 text-xs text-zinc-900 outline-none focus:border-emerald-500"/><div className="flex gap-2"><Button type="button" variant="secondary" size="sm" onClick={()=>setReportProfile(null)} className="flex-1">Cancel</Button><Button type="submit" variant="primary" size="sm" disabled={reporting} className="flex-1 bg-rose-600 text-white">{reporting?"Submitting…":"Report & Block"}</Button></div></form></motion.div></div>}</AnimatePresence>
  <AnimatePresence>{matchModal&&<div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md"><motion.div initial={{scale:.85,opacity:0,y:20}} animate={{scale:1,y:0,opacity:1}} className="relative w-full max-w-sm rounded-3xl bg-card border border-border p-7 text-center shadow-2xl space-y-4"><button type="button" onClick={()=>setMatchModal(null)} className="absolute top-4 right-4 p-2 rounded-full text-muted-foreground bg-muted"><X className="h-4 w-4"/></button><div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold"><Sparkles className="h-3.5 w-3.5"/>Mutual Connection!</div><h2 className="text-2xl font-black text-foreground">It&apos;s a Match!</h2><p className="text-xs text-muted-foreground">You and <span className="text-emerald-600 font-bold">{matchModal.profile.display_name}</span> liked each other.</p><Link href={matchModal.matchId?`${routes.messages}/${matchModal.matchId}`:routes.matches} className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-full bg-emerald-600 text-white text-xs font-bold"><MessageCircle className="h-4 w-4"/>Send First Message</Link><Link href={`${routes.profileView}/${matchModal.profile.id}`} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-full border border-border text-xs font-bold"><UserRound className="h-4 w-4"/>View Profile</Link><button type="button" onClick={()=>setMatchModal(null)} className="w-full py-2 text-xs font-semibold text-muted-foreground">Keep Swiping</button></motion.div></div>}</AnimatePresence>
 ...
 return <motion.div style={{x:isTop?x:0,rotate:isTop?rotate:0,transform:"translate3d(0,0,0)",WebkitTransform:"translate3d(0,0,0)",backfaceVisibility:"hidden",WebkitBackfaceVisibility:"hidden",willChange:"transform"}} animate={{scale:isTop?1:.96,y:isTop?0:8,opacity:isTop?1:.65}} transition={{type:"spring",stiffness:400,damping:32}} drag={isTop?"x":false} dragConstraints={{left:0,right:0}} dragElastic={.65} onDragEnd={dragEnd} className={`absolute inset-0 overflow-hidden rounded-[2rem] select-none ${isTop?"z-20 cursor-grab active:cursor-grabbing touch-pan-y":"z-10 pointer-events-none"}`}>
  <div className="relative h-full w-full overflow-hidden rounded-[2rem] bg-zinc-950 shadow-2xl">
   {!isTop?<div className="absolute inset-0 border border-zinc-800 bg-zinc-900"/>:<>
    ...
    <div className="absolute top-6 right-3.5 z-40 flex items-center gap-1.5"><div className="flex items-center gap-1 rounded-full bg-black/60 px-2.5 py-1 text-[10px] font-bold text-emerald-400 border border-white/15 pointer-events-none"><ShieldCheck className="h-3 w-3"/>Verified</div><button type="button" onClick={e=>{e.stopPropagation();onOpenSafety()}} className="flex h-8 w-8 items-center justify-center rounded-full bg-black/65 text-white border border-white/15 shadow-sm" aria-label="Safety controls" title="Safety controls"><MoreVertical className="h-4 w-4"/></button></div>
    <div className="absolute bottom-3 left-4 right-4 z-30 space-y-1.5 pointer-events-none">
      ...
      <div className="pointer-events-auto flex items-center justify-between gap-2 border-t border-white/15 pt-2"><span className="text-[9px] font-semibold text-white/55">← Pass</span><button type="button" onClick={e=>{e.stopPropagation();onSuperChat()}} className="inline-flex items-center gap-1.5 rounded-full border border-violet-300/40 bg-violet-600 px-3.5 py-1.5 text-[10px] font-black text-white shadow-lg shadow-violet-950/30 ring-1 ring-white/10" aria-label={`Send SuperChat · ₹${SHOP_PRODUCTS.superchat.amountPaise/100}`}><MessageCircle className="h-3.5 w-3.5"/>SuperChat · ₹{SHOP_PRODUCTS.superchat.amountPaise/100}</button><span className="text-[9px] font-semibold text-white/55">Like →</span></div>
    </div>
   </>}
  </div>
 </motion.div>;

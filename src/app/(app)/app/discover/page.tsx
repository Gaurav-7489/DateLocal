...
export default async function DiscoverPage() {
  ...
  if (!myProfile?.profile_completed) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        <Card className="border-pink-200 bg-pink-50/50 p-8">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-pink-100 text-pink-600"><Sparkles className="h-7 w-7" /></div>
          <h1 className="mt-4 text-2xl font-black">Complete your Extrovert profile first</h1>
          <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">You can use Extrovert socially without dating. Complete the dating profile only when you want to appear in dating discovery.</p>
          <Link href={routes.profileSetup}><Button className="mt-4 gap-2 bg-pink-500 hover:bg-pink-600">Set up dating <ArrowRight className="h-4 w-4" /></Button></Link>
        </Card>
      </div>
    );
  }
  ...
  if (error) return <div className="mx-auto max-w-md px-4 py-16 text-center"><h1 className="text-xl font-bold">Discover is taking a moment</h1><p className="mt-2 text-sm text-muted-foreground">We couldn&apos;t load discovery right now. Please try again.</p></div>;

  let normalized = (rawProfiles ?? []) as DiscoverProfile[];
  const wanted = Array.isArray(myPrefs?.interested_in) ? myPrefs.interested_in : [];
  if (!wanted.includes("everyone") && wanted.length) {
    normalized = normalized.filter((p) => {
      const g = (p.gender || "").toLowerCase();
      if (wanted.includes("men") && (g === "man" || g === "male")) return true;
      if (wanted.includes("women") && (g === "woman" || g === "female")) return true;
      if ((wanted.includes("nonbinary") || wanted.includes("other")) && (g === "non-binary" || g === "nonbinary" || g === "other")) return true;
      return false;
    });
  }
  ...
  const ranked = normalized
    .map((profile, index) => {
      const trust = trustMap.get(profile.id);
      const verified = trust?.verification_status === "verified";
      const areaVerified = trust?.area_verification_status === "verified";
      const trustBoost = (verified ? VERIFIED_REACH_BOOST : 0) + (areaVerified ? VERIFIED_REACH_BOOST / 2 : 0);
      return { profile, index, score: index - trustBoost - (beyondIds.has(profile.id) ? 0.15 : 0) };
    })
    .sort((a, b) => a.score - b.score)
    .slice(0, DISCOVER_BATCH_SIZE)
    .map((item) => item.profile);

  const profilesWithPhotoUrls = ranked.map((profile) => {
    const trust = trustMap.get(profile.id);
    const photos = [...(profile.profile_photos ?? [])].sort((a, b) => Number(b.is_primary) - Number(a.is_primary) || a.display_order - b.display_order).slice(0, 5).map((photo) => ({ ...photo, url: getProfilePhotoUrl(photo.storage_path, 1080) }));
    const sharedPhoto = getProfilePhotoUrl(trust?.profile_photo_path, 1080);
    const context = profile.identity_type === "student" ? `${profile.institution_name || "College / university"} · ${profile.department || "Student"} · ${profile.academic_year || "Current year"}` : profile.identity_type === "professional" ? `${profile.job_title || "Professional"}${profile.employer_name ? ` · ${profile.employer_name}` : ""}` : (profile.role_description || "Current role");
    const trustLabel = trust?.verification_status === "verified" ? "Identity: Verified" : "Identity: Not verified";
    const areaLabel = trust?.area_verification_status === "verified" && trust.area_id ? `Area: ${areaMap.get(trust.area_id) ?? "Verified"}` : "Area: Not verified";
    const beyond = profile.gender === "woman" || beyondIds.has(profile.id);
    return { ...profile, profile_photo_url: photos[0]?.url ?? sharedPhoto, profile_photos: photos, verification_status: trust?.verification_status ?? "not_verified", area_verification_status: trust?.area_verification_status ?? "not_verified", area_name: trust?.area_id ? areaMap.get(trust.area_id) ?? null : null, bio: `${beyond ? "Beyond · " : ""}${trustLabel} · ${areaLabel} · ${context}${profile.bio ? ` — ${profile.bio}` : ""}` };
  });

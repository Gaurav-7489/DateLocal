...
export default async function DiscoverPage() {
  ...
  // Everything here is independent and can be fetched together. This removes
  // the extra isPro round-trip that previously delayed the first deck render.
  const [
    { data: myProfile },
    { data: myPrefs },
    { data: likes },
    ...
    supabase.from("dating_preferences").select("preferred_department").eq("user_id", userId).maybeSingle(),
  ...
  if (!myProfile?.profile_completed) {
    return <div className="mx-auto max-w-2xl px-4 py-16 text-center"><Card className="border-emerald-200 bg-emerald-50/50 p-8"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700"><Sparkles className="h-7 w-7" /></div><h1 className="mt-4 text-2xl font-black">Create your dating profile first</h1><p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">Your Extrovert identity can exist without dating. Create your dating profile to appear in Extrovert Date and discover other daters.</p><Link href={routes.profileSetup}><Button className="mt-4 gap-2">Create dating profile <ArrowRight className="h-4 w-4" /></Button></Link></Card></div>;
  }
  ...
  const { data: rawProfiles, error } = await supabase.rpc("get_discover_profiles", {
  ...
  const [{ data: extrovertIds }, { data: paidRows }] = await Promise.all([
  ...
  const proIds = new Set((extrovertIds ?? []) as string[]);
  ...
  const ranked = normalized
    .map((profile, index) => ({ profile, index, score: proIds.has(profile.id) ? index / EXTROVERT_REACH_BOOST : index }))
  ...
  const profilesWithPhotoUrls = ranked.map((profile) => {
    const photos = [...(profile.profile_photos ?? [])]
      ...
      .slice(0, 5)
      .map((photo) => ({ ...photo, url: getProfilePhotoUrl(photo.storage_path, 1080) }));
    const context = profile.identity_type === "student"
      ? `${profile.institution_name || "College / university"} · ${profile.department || "Student"} · ${profile.academic_year || "Current year"}`
      : profile.identity_type === "professional"
        ? `${profile.job_title || "Professional"}${profile.employer_name ? ` · ${profile.employer_name}` : ""}`
        : (profile.role_description || "Current role");
    const beyond = profile.gender === "woman" || beyondIds.has(profile.id);
    const bio = `${beyond ? "Beyond · " : ""}${context}${profile.bio ? ` — ${profile.bio}` : ""}`;
    return { ...profile, bio, profile_photo_url: photos[0]?.url ?? null, profile_photos: photos };
  });

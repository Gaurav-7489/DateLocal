...
export async function likeProfile(profileId: string): Promise<LikeResult> {
  ...
  // The database RPC owns the allowance and like transaction. This prevents
  // double-taps/races from bypassing the 7 starter / 2 daily / 10 Extrovert limits.
  ...
  if (error) {
    ...
    if (message.includes("PROFILE_UNAVAILABLE")) return { error: "That profile is no longer available.", matched: false };
    if (message.includes("USER_UNAVAILABLE")) return { error: "This user is unavailable.", matched: false };
    if (message.includes("AUTH_REQUIRED")) return { error: "You must be logged in to like someone.", matched: false };
    if (message.includes("LIKE_LIMIT_REACHED")) return { error: "You're out of likes for now. You can get more in the DateBu Shop or unlock Extrovert.", matched: false };
  ...
  if (matched) { revalidatePath(routes.matches); revalidatePath(routes.messages); }

  const { data: targetProfile } = await supabase.from("profiles").select("display_name").eq("id", profileId).maybeSingle();
  const targetName = targetProfile?.display_name || "someone";
  if (matched && matchId) {
    await Promise.all([
      sendPushToUser(profileId, { title: "It’s a Match!", body: "You matched on DateBu.", url: `${routes.messages}/${matchId}`, tag: `match-${matchId}` }),
      sendPushToUser(user.id, { title: "It’s a Match!", body: `You matched with ${targetName}.`, url: `${routes.messages}/${matchId}`, tag: `match-${matchId}` }),
    ]);
  } else {
    await sendPushToUser(profileId, { title: "Someone likes you", body: "Someone liked your profile. Open DateBu to see who.", url: routes.discover, tag: `like-${user.id}` });
  }
...
export async function passProfile(profileId: string): Promise<ActionResult> {
  ...
  const { error } = await supabase.from("passes").insert({ passer_id: user.id, passed_id: profileId });
  if (error && error.code !== "23505") { console.error("Pass failed:", error); return { error: "Couldn't save your pass. Please try again." }; }
  revalidatePath(routes.discover);
...
export async function resetPassedProfiles(): Promise<ActionResult> {
  ...
  if (error) { console.error("Failed to reset passed profiles:", error); return { error: "Couldn't bring back passed profiles. Please try again." }; }
  revalidatePath(routes.discover);
  return { error: null, success: true };
}
...
export async function blockUser(targetUserId: string): Promise<ActionResult> {
  ...
  if (error && error.code !== "23505") { console.error("Block failed:", error); return { error: "Failed to block user. Please try again." }; }
  ...
  revalidatePath(routes.discover); revalidatePath(routes.matches); revalidatePath(routes.messages); revalidatePath(routes.settings);
  return { error: null, success: true };
}
...
export async function unblockUser(targetUserId: string): Promise<ActionResult> {
  ...
  revalidatePath(routes.settings); revalidatePath(routes.discover);
  return { error: null, success: true };
}
...
export async function reportUser(targetUserId: string, reason: string, details?: string): Promise<ActionResult> {
  ...
  const { error: reportError } = await supabase.from("reports").insert({ reporter_id: user.id, reported_id: targetUserId, reason: reason.trim(), details: details?.trim() || null });
  if (reportError) { console.error("Report failed:", reportError); return { error: "Failed to submit report. Please try again." }; }
  await supabase.from("blocks").upsert({ blocker_id: user.id, blocked_id: targetUserId }, { onConflict: "blocker_id,blocked_id" });
  revalidatePath(routes.discover); revalidatePath(routes.matches);
  return { error: null, success: true };
}
...
export async function toggleGhostMode(enabled: boolean): Promise<ActionResult> {
  ...
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: "You must be logged in." };
  const { data: subscription } = await supabase.from("subscriptions").select("status, plan").eq("user_id", user.id).maybeSingle();
  if (!subscription || subscription.status !== "active" || subscription.plan === "free") return { error: "Ghost Mode is a DateBu Extrovert feature." };
  const { error } = await supabase.from("profiles").update({ ghost_mode: enabled }).eq("id", user.id);
  if (error) return { error: "Failed to update ghost mode." };
  revalidatePath(routes.settings); revalidatePath(routes.profile); revalidatePath(routes.app);
  return { error: null, success: true };
}

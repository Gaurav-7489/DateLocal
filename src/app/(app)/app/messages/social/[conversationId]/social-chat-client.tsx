...
export default function SocialChatClient({ conversationId, currentUserId, otherUserId, initialMessages }: Props) {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      try { await ensureOwnMessageKey(supabase, currentUserId); if (alive) setReady(true); }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`social_chat_${conversationId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "extrovert_messages", filter: `conversation_id=eq.${conversationId}` }, async payload => {
  ...
  }, [ready, conversationId, currentUserId, otherUserId, supabase]);
  ...
  async function handleSend(event: React.FormEvent) {
    ...
    try {
      ...
      const optimistic: Message = { id: tempId, sender_id: currentUserId, ciphertext, content: text, created_at: new Date().toISOString() };
      setMessages(prev => [...prev, optimistic]); setContent("");
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-white">
    ...
    <div className="flex-1 space-y-3 overflow-y-auto px-3 py-4 no-scrollbar">
      {messages.map(message => { const mine = message.sender_id === currentUserId; return <div key={message.id} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}><div className={`max-w-[82%] rounded-2xl px-3.5 py-2 text-xs leading-relaxed shadow-xs ${mine ? "rounded-br-sm bg-emerald-600 text-white" : "rounded-bl-sm border border-zinc-200 bg-white text-zinc-900"}`}><p className="whitespace-pre-wrap break-words">{message.content ?? "Decrypting…"}</p></div><span className="mt-0.5 px-1.5 text-[9px] text-zinc-400">{formatTime(message.created_at)}</span></div>; })}
      {messages.length === 0 && <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-8 text-center text-xs text-zinc-500">No messages yet. Start the conversation.</div>}
      <div ref={bottomRef} />
    </div>

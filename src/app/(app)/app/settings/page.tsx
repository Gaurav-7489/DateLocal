...
export default async function SettingsPage() {
  ...
  const { data: preferences } = await supabase
    ...
    .select(
      "interested_in, min_age, max_age, preferred_department",
    )
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  // Load DateBu Extrovert subscription
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Manage your account, privacy, discovery visibility,
          and safety controls
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1 text-xs">
          <div className="rounded-2xl bg-muted/50 p-3">
            ...
            <span className="font-semibold text-foreground capitalize mt-0.5 block">
              {preferences?.interested_in?.join(", ") ||
                "Everyone"}
            </span>
          </div>
          ...
          <div className="rounded-2xl bg-muted/50 p-3">
            ...
            <span className="font-semibold text-foreground mt-0.5 block">
              {preferences?.min_age ?? 18} –{" "}
              {preferences?.max_age ?? 25} yrs
            </span>
          </div>
      ...
      <Card className="p-6 space-y-3">
        ...
        <div className="divide-y divide-border text-xs">
          ...
          >
            <span>
              Campus Safety Center &amp; Guidelines
            </span>

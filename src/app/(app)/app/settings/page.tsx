...
import { SlidersHorizontal, LogOut, ExternalLink, Lock, Link2, ShieldCheck, MapPin, UserRound } from "lucide-react";
...
export default async function SettingsPage() {
  ...
  const [{data:identity},{data:preferences},{data:profile},{data:blocks},{data:subscription},{data:area}]=await Promise.all([
    supabase.from("extrovert_profiles").select("display_name,gender,area_id,department,academic_year,verification_status,area_verification_status,trust_state").eq("id",user.id).maybeSingle(),
    ...
    identity?.area_id ? supabase.from("extrovert_areas").select("name").eq("id",identity.area_id).maybeSingle() : Promise.resolve({data:null}),
  ]);
  ...
  const showMe=preferences?.interested_in?.includes("everyone")?"Everyone":preferences?.interested_in?.includes("men")&&preferences?.interested_in?.includes("women")?"Everyone":preferences?.interested_in?.[0]?preferences.interested_in[0].replace(/^./,(c:string)=>c.toUpperCase()):"Everyone";
  const extrovertUrl=process.env.NEXT_PUBLIC_EXTROVERT_URL||PRODUCTION_EXTROVERT_URL;
  return <div className="mx-auto max-w-md px-3.5 py-4 space-y-4 font-sans select-none pb-24"><div className="px-1"><p className="text-[10px] font-black uppercase tracking-[.16em] text-pink-600">EXTROVERT DATE</p><h1 className="text-2xl font-black tracking-tight">Settings</h1><p className="text-[11px] text-muted-foreground font-medium">Dating controls, privacy and account safety</p></div>
    <section className="rounded-3xl border border-pink-200/70 bg-pink-50/50 p-4 shadow-xs"><div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-pink-600"><Link2 className="h-4 w-4"/></div><div className="min-w-0 flex-1"><p className="text-xs font-black text-pink-950">Shared identity · Extrovert</p><p className="mt-1 text-[10px] leading-4 text-pink-800">{identity?.display_name||"Your identity"}{identity?.gender?` · ${identity.gender}`:""}{identity?.department?` · ${identity.department}`:""}{identity?.academic_year?` · ${identity.academic_year}`:""}</p><p className="mt-1 flex items-center gap-1 text-[10px] text-pink-800"><MapPin className="h-3 w-3"/>{area?.name||"Local area"}</p><div className="mt-2 flex flex-wrap gap-1"><TrustBadge ok={identity?.verification_status==="verified"} yes="Identity verified" no="Identity not verified"/><TrustBadge ok={identity?.area_verification_status==="verified"} yes="Area verified" no="Area not verified"/></div><p className="mt-2 text-[9px] leading-4 text-pink-800">Identity, gender, local area, profile photo and verification are shared with Extrovert Date.</p><a href={extrovertUrl} className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold text-pink-700">Edit in Extrovert <ExternalLink className="h-3 w-3"/></a></div></div></section>
    <section className="rounded-3xl border border-border/80 bg-card p-4 space-y-3 shadow-xs"><div className="flex items-center justify-between"><span className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground flex items-center gap-1"><SlidersHorizontal className="w-3.5 h-3.5 text-pink-600"/> Dating & Matching</span><Link href={routes.profileSetup} className="text-xs font-bold text-pink-600">Edit</Link></div><div className="grid grid-cols-3 gap-2 pt-1 text-center"><div className="rounded-2xl bg-muted/40 p-2.5 border border-border/60"><span className="text-[9px] uppercase font-bold text-muted-foreground block">Show me</span><span className="text-xs font-extrabold block truncate">{showMe}</span></div><div className="rounded-2xl bg-muted/40 p-2.5 border border-border/60"><span className="text-[9px] uppercase font-bold text-muted-foreground block">Age</span><span className="text-xs font-extrabold block">{preferences?.min_age??18}–{preferences?.max_age??25}</span></div><div className="rounded-2xl bg-muted/40 p-2.5 border border-border/60"><span className="text-[9px] uppercase font-bold text-muted-foreground block">Dept</span><span className="text-xs font-extrabold block truncate">{preferences?.preferred_department||"All"}</span></div></div></section>

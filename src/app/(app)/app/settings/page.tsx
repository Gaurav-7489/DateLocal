...
import { SlidersHorizontal, LogOut, ExternalLink, Lock, ShieldCheck, MapPin, MessageSquare, Bot } from "lucide-react";
...
export default async function SettingsPage() {
  ...
  return <div className="mx-auto max-w-md space-y-4 px-3.5 py-4 pb-24 font-sans select-none">
    ...
    <section className="divide-y divide-border/60 overflow-hidden rounded-3xl border border-border/80 bg-card shadow-xs"><div className="bg-muted/30 px-4 py-2.5"><span className="flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground"><Lock className="h-3 w-3" /> Safety & Support</span></div><Link href={routes.support} className="flex items-center justify-between p-3.5 text-xs font-semibold"><span className="flex items-center gap-2"><Bot className="h-3.5 w-3.5 text-emerald-600" />Support</span><ExternalLink className="h-3.5 w-3.5 text-muted-foreground" /></Link><Link href={routes.feedback} className="flex items-center justify-between p-3.5 text-xs font-semibold"><span className="flex items-center gap-2"><MessageSquare className="h-3.5 w-3.5 text-emerald-600" />Contact & Feedback</span><ExternalLink className="h-3.5 w-3.5 text-muted-foreground" /></Link><Link href={routes.safety} className="flex items-center justify-between p-3.5 text-xs font-semibold"><span>Safety Center</span><ExternalLink className="h-3.5 w-3.5 text-muted-foreground" /></Link><Link href={routes.privacy} className="flex items-center justify-between p-3.5 text-xs font-semibold"><span>Privacy & Data Rights</span><ExternalLink className="h-3.5 w-3.5 text-muted-foreground" /></Link><Link href={routes.terms} className="flex items-center justify-between p-3.5 text-xs font-semibold"><span>Terms of Service</span><ExternalLink className="h-3.5 w-3.5 text-muted-foreground" /></Link></section>

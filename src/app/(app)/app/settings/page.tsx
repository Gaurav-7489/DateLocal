...
export default async function SettingsPage() {
  ...
  return <div className="mx-auto max-w-md px-3.5 py-4 space-y-4 font-sans select-none pb-24">
    ...
    <section className="rounded-3xl border border-emerald-200/70 bg-emerald-50/50 p-4 shadow-xs"><div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-emerald-700"><Link2 className="h-4 w-4"/></div><div className="min-w-0 flex-1"><p className="text-xs font-black text-emerald-950">Shared identity · Extrovert</p><p className="mt-1 text-[10px] leading-4 text-emerald-800">{identity?.display_name||"Your identity"}{identity?.department?` · ${identity.department}`:""}{identity?.academic_year?` · ${identity.academic_year}`:""}</p><p className="mt-1 text-[9px] text-emerald-800">Identity, student details, local area and verification are managed in Extrovert.</p><a href={extrovertUrl} className="mt-2 inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700">Edit in Extrovert <ExternalLink className="h-3 w-3"/></a></div></div></section>

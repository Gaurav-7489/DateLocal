...
export default async function BeyondPage() {
  ...
  const [{ data: subscription }] = await Promise.all([user ? supabase.from("subscriptions").select("plan,status,current_period_end").eq("user_id", user.id).maybeSingle() : Promise.resolve({ data: null })]);
  ...
  return (
    <main className="mx-auto max-w-md px-3.5 py-4 pb-28 font-sans">
      <div className="flex items-center gap-3 px-1"><Link href={routes.app} className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-card text-muted-foreground shadow-xs active:scale-95" aria-label="Back"><ArrowLeft className="h-4 w-4" /></Link><div><p className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-600">EXTROVERT</p><h1 className="text-xl font-black tracking-tight">Beyond</h1></div></div>
      <section className="mt-4 rounded-[2rem] border border-violet-200/80 bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 p-5 shadow-sm"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.16em] text-violet-700"><Sparkles className="h-3.5 w-3.5" />Beyond</div><h2 className="mt-2 text-3xl font-black tracking-tight">More from your Extrovert experience.</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">Beyond is the optional premium layer inside Extrovert. Your core social and dating identity stays the same.</p>{active?<div className="mt-4 rounded-2xl border border-violet-200 bg-violet-100/70 p-3"><p className="text-xs font-black text-violet-900">Beyond is active</p></div>:<DateBuExtrovertCheckout email={user?.email} />}</section>

      ...
      <div className="mx-auto flex min-h-[calc(100dvh-3rem)] w-full max-w-md items-center justify-center">
        <section className="w-full rounded-[28px] border border-zinc-200 bg-white p-5 shadow-[0_16px_50px_rgba(0,0,0,0.07)] sm:p-7">
          ...
          <form onSubmit={handleEmail} className="space-y-3">
            ...
            <label className="block text-xs font-bold text-zinc-700">
              ...
              <span className="relative block">
                <input type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Your password" className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3.5 py-3 pr-12 text-sm outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/15" required />
          ...
          <div className="mt-4 flex justify-center gap-4 text-xs font-semibold">
            <Link href={routes.resetPassword} className="text-emerald-700 hover:underline">Forgot password?</Link>
            <Link href={routes.register} className="text-zinc-600 hover:text-zinc-950 hover:underline">Create account</Link>
          </div>
        </section>
      </div>
    </main>

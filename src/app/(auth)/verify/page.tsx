...
function VerifyPageWithParams() {
  ...
  return <VerifyPageContent initialEmail={searchParams.get("email")?.trim().toLowerCase() ?? ""} error={searchParams.get("error") ?? ""} />;
}

function VerifyPageContent({ initialEmail, error }: { initialEmail: string; error: string }) {
  const router = useRouter();
  const [email, setEmail] = useState(initialEmail);
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState<"idle" | "success" | "error">(error ? "error" : "idle");
  ...
  return <main className="relative min-h-[100dvh] overflow-hidden bg-[#f7fbf9] px-4 py-6 font-sans text-zinc-900 antialiased">
    <header className="relative z-10 mx-auto flex w-full max-w-md items-center justify-between rounded-2xl border border-zinc-200/80 bg-white px-4 py-3 shadow-sm">
      <Link href={routes.home} className="flex items-center gap-2"><div className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-600 text-xs font-black text-white">E</div><span className="text-xs font-extrabold tracking-tight">Extrovert</span></Link>
    ...
    <section className="relative z-10 mx-auto flex min-h-[calc(100dvh-8rem)] w-full max-w-md items-center justify-center py-8"><div className="w-full rounded-3xl border border-zinc-200/90 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      ...
      <div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50/50 p-3.5"><p className="flex items-center gap-1 text-[11px] font-bold text-emerald-950"><Sparkles className="h-3 w-3 text-emerald-600" /> What happens next</p><ol className="mt-2 list-decimal space-y-1.5 pl-4 text-[11px] leading-5 text-zinc-600"><li>Open the email from <strong>Extrovert</strong>.</li><li>Tap the confirmation link.</li><li>You’ll return to Extrovert.</li><li>Finish your profile setup.</li></ol></div>
      {status !== "idle" && <div className={`mt-4 flex items-center gap-1.5 rounded-xl p-2 text-[11px] ${status === "success" ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-rose-200 bg-rose-50 text-rose-700"}`}>{status === "success" ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> : <AlertCircle className="h-3.5 w-3.5 shrink-0" />}<span>{message}</span></div>}
      ...
      <div className="mt-4 border-t border-zinc-100 pt-4"><p className="mb-2 text-center text-[11px] font-semibold text-zinc-700">Didn’t receive the email?</p><form onSubmit={handleResend} className="space-y-2"><input type="email" placeholder="you@example.com" value={email} onChange={(event) => setEmail(event.target.value)} required className="w-full rounded-xl border border-zinc-200 bg-zinc-50/70 px-3.5 py-2.5 text-xs outline-none focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/20" /><button type="submit" disabled={loading || !email.trim() || countdown > 0} className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-emerald-600 py-3 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{loading ? <><Loader2 className="h-3 w-3 animate-spin" /> Sending link...</> : countdown > 0 ? <><RefreshCw className="h-3 w-3" /> Resend in {countdown}s</> : <><RefreshCw className="h-3 w-3" /> Send New Verification Link</>}</button></form></div>
    </div></section>
  </main>;
}

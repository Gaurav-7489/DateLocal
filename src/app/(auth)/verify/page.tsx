...
function VerifyPageContent({ initialEmail }: { initialEmail: string }) {
  ...
  return (
    <main className="relative min-h-[100dvh] overflow-hidden bg-[#f7fbf9] px-4 py-6 font-sans text-zinc-900 antialiased">
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="absolute left-1/2 top-1/4 h-[380px] w-[380px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-500/15 blur-[140px]" />
      </div>

      <header className="relative z-10 mx-auto flex w-full max-w-md items-center justify-between rounded-2xl border border-zinc-200/80 bg-white/85 px-4 py-3 shadow-sm backdrop-blur-xl">
        <Link href="/" className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-600 text-xs font-black text-white">
            {universityConfig.shortName[0]}
          </div>
          <span className="text-xs font-extrabold tracking-tight">{universityConfig.appName}</span>
        </Link>
        <Link href={routes.login} className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-700">
          Sign In
        </Link>
      </header>

      <section className="relative z-10 mx-auto flex min-h-[calc(100dvh-8rem)] w-full max-w-md items-center justify-center py-8">
        <div className="w-full rounded-3xl border border-zinc-200/90 bg-white/95 p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-xl">
          ...
          {email && (
            <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 px-3.5 py-3 text-center text-xs font-semibold text-zinc-700">
              {email}
            </div>
          )}
          ...
          <div className="mt-4 border-t border-zinc-100 pt-4">
            <p className="mb-2 text-center text-[11px] font-semibold text-zinc-700">Didn&apos;t receive the email?</p>
            <form onSubmit={handleResend} className="space-y-2">
              <input
                ...
                required
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50/70 px-3.5 py-2.5 text-xs outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/20"
              />
              ...
              {status !== "idle" && (
                <motion.div
                  ...
                  className={`flex items-center gap-1.5 rounded-xl p-2 text-[11px] ${status === "success" ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-rose-200 bg-rose-50 text-rose-700"}`}
                >
                  {status === "success" ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> : <AlertCircle className="h-3.5 w-3.5 shrink-0" />}
                  <span>{message}</span>
                </motion.div>
              )}

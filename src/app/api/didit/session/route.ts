...
async function imageUrlToBase64(imageUrl: string): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error("Unable to load your primary profile photo.");
  }

  const contentLength = Number(response.headers.get("content-length") ?? "0");
  if (contentLength > MAX_PORTRAIT_BYTES) {
    throw new Error("Your primary profile photo is too large for verification. Please use a smaller photo.");
  }
...
export async function POST() {
  try {
    ...
    // Didit Face Match expects the reference portrait as raw Base64 and caps it at 2 MB.
    const portraitImage = await imageUrlToBase64(profilePhotoUrl);
    ...
    if (!workflowId) {
      return NextResponse.json(
        { error: "Face verification is not configured yet. Please try again later." },
        { status: 503 },
      );
    }
    ...
    const diditResult = await createDiditSession({
      ...
      portraitImage,
      metadata: { datebu_user_id: user.id },
    });

...
export default function SuperChatComposer({ targetUserId, targetName, onClose, onComplete }: Props) {
  ...
  async function send() {
    ...
    try {
      ...
      const instance = new Razorpay({ key: data.keyId, amount: data.amount, currency: data.currency, order_id: data.orderId, name: "DateBu", description: `SuperChat to ${targetName}`, theme: { color: "#10b981" }, config: UPI_ONLY_CONFIG, modal: { ondismiss: () => setLoading(false) }, handler: async (payment) => {
      ...
      instance.on?.("payment.failed", () => { setError("UPI payment was declined. Your account was not charged."); setLoading(false); });
      instance.open();

      ...
      <div className="flex items-center justify-between gap-4 rounded-2xl border border-border/70 bg-muted/20 p-3.5">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[10px] text-muted-foreground">
              {standalone ? "DateBu is installed on this device" : ios ? "Use Share → Add to Home Screen" : installAvailable ? "Ready to install as an app" : "Install availability depends on your browser"}
            </p>
          </div>
        </div>
        {standalone ? (
          <span className="shrink-0 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300">Installed</span>
        ) : (
          <InstallPwaButton />
        )}
      </div>

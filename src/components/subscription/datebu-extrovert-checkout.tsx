...
> = {
  weekly: {
    name: "Weekly Pass",
    price: 39,
    description: "Full DateBu Extrovert perks for 7 days",
  },
  monthly: {
    name: "Monthly Pass",
    price: 99,
    description: "Uncapped visibility & Ghost Mode for 30 days",
    badge: "Most Popular",
  },
};
...
export function DateBuExtrovertCheckout({
  ...
  async function startCheckout(plan: BillingPlan) {
    try {
      ...
      if (!scriptLoaded) {
        throw new Error(
          "Unable to load Razorpay Checkout. Please check your network connection.",
        );
      }
      ...
      if (!data.keyId || !data.subscriptionId) {
        throw new Error("Gateway issued an incomplete subscription parameter.");
      }
  ...
  return (
    <div className="space-y-5 font-sans">
        ...
        {(Object.keys(PLANS) as BillingPlan[]).map((plan) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 rounded-2xl bg-zinc-950 py-3 px-4 text-xs font-bold text-white transition-all group-hover:bg-emerald-600 shadow-sm">
                {isLoading ? (
                  <>
      ...
      {error && (
        <div
          role="alert"
          className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs font-semibold text-rose-700"
        >
          {error}
        </div>
      )}
